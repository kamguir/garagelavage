...
/**
 * Base class that represents a row from the 'ref_marque' table.
 *
 * 
 *
 ...
 */
abstract class BaseRefMarque extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'RefMarquePeer';
	...
	/**
	 * The value for the marque_id field.
	 * @var        int
	 */
	protected $marque_id;

	/**
	 * The value for the marque_libelle field.
	 * @var        string
	 */
	protected $marque_libelle;
	...
	/**
	 * Get the [marque_id] column value.
	 * 
	 * @return     int
	 */
	...
	/**
	 * Get the [marque_libelle] column value.
	 * 
	 * @return     string
	 */
	...
	/**
	 * Set the value of [marque_id] column.
	 * 
	 * @param      int $v new value
	 * @return     RefMarque The current object (for fluent API support)
	 */
	public function setMarqueId($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}
		...
		if ($this->marque_id !== $v) {
			$this->marque_id = $v;
			$this->modifiedColumns[] = RefMarquePeer::MARQUE_ID;
		}
	...
	/**
	 * Set the value of [marque_libelle] column.
	 * 
	 * @param      string $v new value
	 * @return     RefMarque The current object (for fluent API support)
	 */
	public function setMarqueLibelle($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->marque_libelle !== $v) {
			$this->marque_libelle = $v;
			$this->modifiedColumns[] = RefMarquePeer::MARQUE_LIBELLE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->marque_id = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->marque_libelle = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = RefMarquePeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating RefMarque object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = RefMarquePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = RefMarqueQuery::create()
				->filterByPrimaryKey($this->getPrimaryKey());
			$ret = $this->preDelete($con);
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRefMarque:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRefMarque:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRefMarque:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRefMarque:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				RefMarquePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		$this->modifiedColumns[] = RefMarquePeer::MARQUE_ID;
		if (null !== $this->marque_id) {
			throw new PropelException('Cannot insert a value for auto-increment primary key (' . RefMarquePeer::MARQUE_ID . ')');
		}

		 // check the columns in natural order for more readable SQL queries
		if ($this->isColumnModified(RefMarquePeer::MARQUE_ID)) {
			$modifiedColumns[':p' . $index++]  = '`MARQUE_ID`';
		}
		if ($this->isColumnModified(RefMarquePeer::MARQUE_LIBELLE)) {
			$modifiedColumns[':p' . $index++]  = '`MARQUE_LIBELLE`';
		}

		$sql = sprintf(
			'INSERT INTO `ref_marque` (%s) VALUES (%s)',
		...
		try {
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					case '`MARQUE_ID`':
						$stmt->bindValue($identifier, $this->marque_id, PDO::PARAM_INT);
						break;
					case '`MARQUE_LIBELLE`':
						$stmt->bindValue($identifier, $this->marque_libelle, PDO::PARAM_STR);
						break;
				}
			}
			$stmt->execute();
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = RefMarquePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = RefMarquePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['RefMarque'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['RefMarque'][$this->getPrimaryKey()] = true;
		$keys = RefMarquePeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getMarqueId(),
			$keys[1] => $this->getMarqueLibelle(),
		);
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			case 0:
				$this->setMarqueId($value);
				break;
			case 1:
				$this->setMarqueLibelle($value);
				break;
		} // switch()
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setMarqueId($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setMarqueLibelle($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(RefMarquePeer::MARQUE_ID)) $criteria->add(RefMarquePeer::MARQUE_ID, $this->marque_id);
		if ($this->isColumnModified(RefMarquePeer::MARQUE_LIBELLE)) $criteria->add(RefMarquePeer::MARQUE_LIBELLE, $this->marque_libelle);
	...
	{
		$criteria = new Criteria(RefMarquePeer::DATABASE_NAME);
		$criteria->add(RefMarquePeer::MARQUE_ID, $this->marque_id);
	...
	/**
	 * Generic method to set the primary key (marque_id column).
	 *
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of RefMarque (or compatible) type.
	...
	{
		$copyObj->setMarqueLibelle($this->getMarqueLibelle());
		...
		if ($makeNew) {
			$copyObj->setNew(true);
			$copyObj->setMarqueId(NULL); // this is a auto-increment column, so set to default value
		}
	}
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     RefMarque Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     RefMarquePeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new RefMarquePeer();
		}
		return self::$peer;
	}
	 ...
	 * If this RefMarque is new, it will return
	 ...
	 * Otherwise if this RefMarque is new, it will return
	 * an empty collection; or if this RefMarque has previously
	 * been saved, it will retrieve related TblVoitures from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in RefMarque.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	...
	{
		return (string) $this->exportTo(RefMarquePeer::DEFAULT_STRING_FORMAT);
	}
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseRefMarque:' . $name))
		{
